...
describe('Provider Configuration', () => {
  ...
  it('should cache key correctly', async () => {
    ...
    expect(mockProviderConfiguration.cacheKey).toEqual(JSON.stringify(mockProviderConfiguration))
  });
...
describe('Provider', () => {
  ...
  it('configures provider correctly', async () => {
    ...
    await expect(mockProvider.configure({
      security: [{
        username: 'second',
        password: 'seconds',
        id: 'second-id'
      }]
    })).resolves.toEqual(new Provider(mockClient, new ProviderConfiguration('test', [{
      username: 'second',
      password: 'seconds',
      id: 'second-id'
    }])))
  ...
  it('configures provider correctly and merges configuration', async () => {
    ...
    const mockClient = new SuperfaceClient()
    const mockProviderConfiguration = new ProviderConfiguration('test', mockSecurity)
    const mockProvider = new Provider(mockClient, mockProviderConfiguration)
    ...
    await expect(mockProvider.configure({
      ...
      serviceId: 'new'
    })).resolves.toEqual(new Provider(mockClient, new ProviderConfiguration('test', [{ id: 'first-id', digest: 'first-digest' }, {
      username: 'second',
      password: 'seconds',
      id: 'second-id'
    }], 'new')))
