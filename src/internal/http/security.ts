...
import { ApiKeyPlacement, ApiKeySecurityScheme, BasicAuthSecurityScheme, BearerTokenSecurityScheme, DigestSecurityScheme, HttpScheme, SecurityType } from "../providerjson";
...
export function applyApiKeyAuth(context: RequestContext, configuration: SecurityConfiguration & { type: SecurityType.APIKEY }): void {
  switch (configuration.in) {
    ...
    case ApiKeyPlacement.BODY:
      if (typeof context.requestBody !== 'object' || Array.isArray(context.requestBody)) {
        throw new Error(
...
export function applyHttpAuth(context: RequestContext, configuration: SecurityConfiguration & { type: SecurityType.HTTP }): void {
...
export function applyBasicAuth(context: RequestContext, configuration: SecurityConfiguration & { type: SecurityType.HTTP, scheme: HttpScheme.BASIC }): void {
  context.headers.append(
    AUTH_HEADER_NAME,
...
export function applyBearerToken(context: RequestContext, configuration: SecurityConfiguration & { type: SecurityType.HTTP, scheme: HttpScheme.BEARER }): void {
  context.headers.append(
    AUTH_HEADER_NAME,
...
export function applyDigest(_context: RequestContext, _configuration: SecurityConfiguration & { type: SecurityType.HTTP, scheme: HttpScheme.DIGEST }): void {
