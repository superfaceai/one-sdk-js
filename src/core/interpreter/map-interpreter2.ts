  ...
  implements VisitorGenerator<V>
{
  ...
  > {
    ...
    if (this.expectedChildIdentifier !== actualChildIdentifier) {
      throw new UnexpectedError(
        `Sanity check failed in ${this.toString()}: Expected child identifier "${
          this.expectedChildIdentifier
        }" but found "${actualChildIdentifier}"`
      );
    }
  ...
  public return(
    _value: VisitorResultDone | PromiseLike<VisitorResultDone>
  ): Promise<
    IteratorResult<
      VisitorResultExplore | VisitorResultYield,
      VisitorResultDone<V>
  ...
  public override async *visit(): VisitorGenerator<Variables | undefined> {
    ...
    for (const iterVariable of iterable) {
      ...
      if (this.node.kind === 'InlineCall') {
        if ('error' in outcome) {
          throw new UnexpectedError('Inline call threw', {
            node: this.node,
            ast: undefined,
          }); // TODO
        }
...
export class MapInterpreter2<TInput extends NonPrimitive | undefined> {
  ...
  public async *performStream(
    ...
    unknown,
    Result<Variables | undefined, MapInterpreterError>,
    undefined
  > {
    // setup
    const operations: Record<string, OperationDefinitionNode | undefined> =
      Object.fromEntries(
        ast.definitions
    ...
    while (nodeStack.length > 0) {
      ...
      const step: IteratorResult<
        VisitorResultExplore | VisitorResultYield,
        VisitorResultDone
      > = await current.next(lastResult as any);
