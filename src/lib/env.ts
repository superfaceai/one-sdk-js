 ...
 */
export function resolveEnv(str: string, logger?: ILogger): string {
  let value = str;
  ...
  if (str.startsWith('$')) {
    ...
    if (env !== undefined) {
      ...
      if (logger !== undefined) {
        logger.log(
          DEBUG_NAMESPACE,
          `Enviroment variable ${variable} not found`
        );
      }
    }
  }
...
export function resolveEnvRecord<T extends Record<string, unknown>>(
  ...
  for (const [key, value] of Object.entries(record)) {
    if (typeof value === 'string') {
      // replace strings
      result[key] = resolveEnv(value, logger);
    } else if (typeof value === 'object' && value !== null) {
      // recurse objects
      result[key] = resolveEnvRecord(value as Record<string, unknown>, logger);
    } else {
      if (value !== undefined) {
